...
(function(root, factory) {
	...
	ScrollMagic.Scene = function(options) {
		...
		var updateRelativePinSpacer = function() {
			if(_controller && _pin && // well, duh
				...
				( // is width or height relatively sized, but not in relation to body? then we need to recalc.
					((_pinOptions.relSize.width || _pinOptions.relSize.autoFullWidth) && _util.get.width(window) != _util.get.width(_pinOptions.spacer.parentNode)) || (_pinOptions.relSize.height && _util.get.height(
						window) != _util.get.height(_pinOptions.spacer.parentNode)))) {
				updatePinDimensions();
			}
		};
		...
		var onMousewheelOverPin = function(e) {
			if(_controller && _pin && _state === SCENE_STATE_DURING && !_controller.info('isDocument')) { // in pin state
				e.preventDefault();
				_controller._setScrollPos(_controller.info('scrollPos') - ((e.wheelDelta || e[_controller.info('vertical') ?
					'wheelDeltaY' :
					'wheelDeltaX']) / 3 || -e.detail * 30));
			}
		};
	...
	var _util = ScrollMagic._util = (function(window) {
		...
		U.log = function(loglevel) {
			...
			var now = new Date(),
				time = ('0' + now.getHours()).slice(-2) + ':' + ('0' + now.getMinutes()).slice(-2) + ':' + ('0' + now.getSeconds()).slice(-2) + ':' + ('00' + now.getMilliseconds()).slice(
					-3),
